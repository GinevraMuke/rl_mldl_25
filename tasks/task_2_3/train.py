...
def main():
    ...
    if args.algorithm == 'actor-critic':
        print("Using Actor-Critic algorithm.")
        from task_2_3.agent_actorCritic import Agent, Policy
    elif args.algorithm == 'reinforce':
        print("Using REINFORCE algorithm.")
        from task_2_3.agent_Reinforce import Agent, Policy
    else:
    ...
    for episode in range(args.n_episodes):
        ...
        if (episode + 1) % args.print_every == 0:
            print(f'Algorithm: {args.algorithm} | Episode: {episode + 1} | Return: {train_reward}')

    model_dir = "./models"
    os.makedirs(model_dir, exist_ok=True)

    if args.algorithm == 'actor-critic':
        save_path = os.path.join(model_dir, "actor_critic.mdl")
    else:  # reinforce
        save_path = os.path.join(model_dir, "REINFORCE_baseline.mdl")
